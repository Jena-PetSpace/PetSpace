...
> 상태: 디자인 방향 및 Set A~D 사용자 결정 반영 완료, W0~W5 구현·시뮬레이터 검토 완료
...
| 목업 세트 | 구현 wave | 결과 |
|---|---|---|
| Navigation Foundation | W0 | 동일 위계 5탭, 루트 전용 노출, 공용 상태·설정·칩 컴포넌트 적용 |
| Set A — Auth | W1 | provider pending 잠금, 안전한 오류, 약관 재시도, 첫 펫 건너뛰기/재진입 적용 |
| Set B — Pet + MY | W2 | 대표 반려동물 영구 선택 계약, 다음 건강 일정, 관리·상태 위계 적용 |
| Set C — Feed + Social | W3 | `피드`/`커뮤니티`, 작성 검증·10장 제한, 신고·차단·저장 복구 적용 |
| Set D — Health | W4 | 다음 케어, 5종 full task editor, PDF·알림 상태 적용 |
| Set D — Chat | W5 | 1:1/그룹 생성, 텍스트·1/10장·재시도, 관리자/멤버 설정 적용 |

구현 후 각 세트는 iPhone 17 Simulator light/dark에서 재검토하고, 발견한 대비·중복 문구·작은 화면 문제를 같은 wave 허용 범위에서 수정했다. 자동/시뮬레이터 검증은 실제 QA 계정 2개와 실기기 E2E를 대체하지 않는다.
