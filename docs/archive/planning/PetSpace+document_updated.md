...
### 프로젝트 성격
반려동물 케어 + 건강관리 + 커뮤니티 허브 + AI 감정 분석 서비스

### UI 디자인 컨셉
**Community Hub (커뮤니티 허브)** — 멍냥보감, 레딧, 블라인드를 참고한 매거진 + 커뮤니티 + 피드 통합형 레이아웃. JENA 브랜드 컬러(Deep Blue #1E3A5F ~ Coral Red #FF6F61) 기반.

### 핵심 가치 제안
- **정확성**: AI 기반 과학적 감정 분석으로 반려동물의 감정 상태를 객관적으로 파악
- **사용자 경험**: 직관적이고 재미있는 UX로 일상적인 사용 유도
- **커뮤니티**: 반려인들 간의 따뜻한 소통과 정보 공유 플랫폼
- **건강관리**: 예방접종, 건강검진, 체중/감정 트렌드를 한 곳에서 관리
...
### 앱 네비게이션 구조 (5탭)
```
탭1: 홈 🏠        — 반려동물 프로필, 매거진, 커뮤니티 미리보기
탭2: 건강관리 💊   — 건강 수첩, 감정 트렌드, 건강 알림
탭3: AI분석 🧠    — 중앙 FAB, AI 감정 분석 (Gemini API)
탭4: 피드 📱      — 추천피드 / 팔로잉 / 커뮤니티 게시판
탭5: MY 👤        — 프로필, 반려동물 관리, 채팅, 설정
```

### 핵심 기능 (MVP)
1. **AI 감정 분석 기능 (탭3 - 중앙 FAB)**
   - 사진 촬영/업로드 기능
   - 실시간 감정 분석 (기쁨, 슬픔, 불안, 졸림, 호기심)
   - 감정 점수 및 신뢰도 표시
   - 스트레스 지수 산출
   - 행동 권장사항 제공

2. **감정 분석 결과 시각화 (탭3)**
   - 실시간 감정 차트 (바 차트, 레이더 차트)
   - 개인 AI 레포트 생성
   - 히스토리 관리 (일/주/월 단위)

3. **홈 대시보드 (탭1)**
   - 반려동물 프로필 카드 (전환 기능)
   - 카테고리 필터 (인기/매거진/커뮤니티/건강/훈련)
   - 인기 토픽 배너
   - 꿀팁 매거진 (2열 그리드)
   - 커뮤니티 최신글 미리보기

4. **건강관리 (탭2)**
   - 건강 수첩 (예방접종, 건강검진, 진료 기록)
   - 주간/월간 감정 트렌드 차트
   - 체중/식사 기록
   - 건강 알림 (D-day)
...
1. **감정 분석**
   - 홈 화면 또는 하단 네비 중앙 FAB(🧠 AI분석) 터치
   - 반려동물 사진 촬영 (또는 갤러리에서 선택)
   - AI 분석 진행 (약 3-5초, Organic Blob 로딩 애니메이션)
...
### 시나리오 3: 감정 변화 추적 및 건강관리
**상황**: 반려동물의 감정 변화를 장기적으로 추적하고 건강을 관리하는 경우

1. **주간 리포트 확인**
   - 건강관리 탭(탭2)에서 주간 감정 트렌드 차트 확인
   - 최근 7일간 감정 변화 바 차트 확인
   - 평소보다 슬픔 지수가 높았던 날 확인

2. **건강 기록 관리**
   - 건강관리 탭에서 예방접종 D-day 알림 확인
   - 건강검진 완료 기록 업데이트
   - 체중 변화 추적
...
4. **커뮤니티 상담**
   - 우려사항을 커뮤니티 게시판(탭4 > 커뮤니티)에 포스팅
   - 다른 반려인들의 경험담 및 조언 수집
   - 필요시 전문가 상담 예약 (향후 기능)

### 시나리오 4: 커뮤니티 탐색 및 매거진 콘텐츠
**상황**: 반려동물 관련 정보를 찾고 다른 보호자와 소통하는 경우

1. **홈 매거진 탐색**
   - 홈 탭(탭1)에서 꿀팁 매거진 그리드 탐색
   - 인기 토픽 배너 터치하여 토픽 참여
   - 카테고리 필터(건강/훈련/먹거리)로 관심 콘텐츠 필터링

2. **커뮤니티 게시판 활동**
   - 피드 탭(탭4) > 커뮤니티 탭에서 건강Q&A 카테고리 진입
   - 질문 게시글 작성 (증상 설명, 사진 첨부)
   - 다른 보호자 및 전문가의 답변 확인

3. **소셜 피드 상호작용**
   - 피드 탭(탭4) > 추천 탭에서 인기 게시글 탐색
   - 감정 분석 결과가 포함된 게시글 확인
   - 관심 있는 보호자 팔로우
...
### 마일스톤 6: UI 전면 리디자인 — Community Hub (Week 13-16)
경쟁사 분석(멍냥보감, 라이펫, 텔레벳, 핏펫, 레딧, 블라인드)을 반영한 전면 UI 리디자인.

**Phase 1 (Week 13)**: 네비게이션 구조 변경
- 기존 5탭(홈/탐색/게시/감정분석/프로필) → 새 5탭(홈/건강관리/AI분석(FAB)/피드/MY)
- 중앙 FAB을 AI분석 전용 버튼으로 변경 (Deep Blue→Bright Blue 그라데이션)
- JENA 브랜드 컬러 디자인 토큰 적용

**Phase 2 (Week 13-14)**: 홈 탭 리디자인
- 반려동물 프로필 카드 (Deep Blue 그라데이션)
- 카테고리 필터 칩 (가로 스크롤)
- 인기 토픽 배너 (Indigo→Bright Blue 그라데이션)
- 꿀팁 매거진 2열 그리드
- 커뮤니티 최신글 미리보기

**Phase 3 (Week 14)**: 건강관리 탭 신규 생성
- 건강 수첩 (예방접종, 건강검진, 체중 기록)
- 주간 감정 트렌드 미니 차트
- 건강 알림 카드 (D-day)

**Phase 4 (Week 15)**: 피드 탭 통합
- 서브 탭: 추천 / 팔로잉 / 커뮤니티
- 커뮤니티 게시판 (카테고리별 필터)
- 기존 소셜 피드 코드 재활용

**Phase 5 (Week 15-16)**: MY 탭 확장
- 프로필 헤더 (통계 포함)
- 메뉴 리스트 (반려동물 관리, 내 게시글, 채팅, 설정 등)
- 기존 프로필/설정 코드 재활용
...
**기능 요구사항:**
- 하단 네비게이션 바 (홈, 건강관리, AI분석(FAB), 피드, MY)
- 중앙 FAB: AI 감정분석 전용 버튼 (Deep Blue→Bright Blue 그라데이션, 라운드 사각형)
- 공통 AppBar 컴포넌트 (좌측 로고, 우측 알림+채팅)
- 로딩, 에러 상태 공통 처리
- 테마 색상 및 스타일 정의 (JENA 브랜드 컬러)
...
- **UI 아키텍처**: Community Hub 컨셉 (5탭: 홈/건강관리/AI분석(FAB)/피드/MY)
- **디자인 시스템**: JENA 브랜드 컬러 기반 (Deep Blue #1E3A5F ~ Coral Red #FF6F61)
...
-- health_records 테이블 (건강관리 탭용)
CREATE TABLE health_records (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  pet_id UUID REFERENCES pets(id) ON DELETE CASCADE,
  user_id UUID REFERENCES users(id) ON DELETE CASCADE,
  record_type VARCHAR(20) CHECK (record_type IN ('vaccination', 'checkup', 'weight', 'medication')),
...
-- magazines 테이블 (매거진 콘텐츠용)
CREATE TABLE magazines (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  title VARCHAR(200) NOT NULL,
  content TEXT,
  category VARCHAR(30) CHECK (category IN ('health', 'training', 'food', 'place', 'education')),
  thumbnail_url TEXT,
...
-- community_posts 테이블 (커뮤니티 게시판용)
CREATE TABLE community_posts (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  author_id UUID REFERENCES users(id) ON DELETE CASCADE,
  category VARCHAR(30) CHECK (category IN ('free', 'health_qa', 'food_recommend', 'parenting_qa', 'info_share')),
  title VARCHAR(200) NOT NULL,
  content TEXT NOT NULL,
  image_urls TEXT[],
  view_count INTEGER DEFAULT 0,
...
### 2. 감정 분석 플로우
사용자가 반려동물의 감정을 분석하는 핵심 기능 (중앙 FAB 또는 홈 퀵액션에서 진입)

**단계별 설명**
1. **분석 시작**: 하단 네비 중앙 FAB(🧠) 터치 또는 홈 퀵액션 "AI 감정분석" 터치
2. **사진 선택**: 카메라 촬영 또는 갤러리 선택
3. **이미지 전처리**: 자동 크롭 및 품질 확인
4. **AI 분석**: 감정 분석 진행 (Organic Blob 로딩 애니메이션)
5. **결과 표시**: 감정 점수 및 차트 표시
6. **상세 정보**: AI 레포트 및 해석
7. **저장/공유**: 히스토리 저장 또는 피드 탭(탭4)으로 공유
...
3. **해시태그**: 관련 태그 추가
4. **공개 설정**: 전체/팔로워만 선택
5. **게시**: 피드 탭(탭4)에 포스트 발행
6. **상호작용**: 좋아요, 댓글 받기

### 4. 커뮤니티 탐색 플로우
다른 사용자의 포스트를 탐색하고 상호작용하는 과정 (피드 탭4에서 진입)

**단계별 설명**
1. **피드 접근**: 하단 네비 피드 탭(탭4) 선택
2. **서브 탭 선택**: 추천 / 팔로잉 / 커뮤니티 중 선택
3. **포스트 탐색**: 스크롤하며 포스트 확인
4. **상세 보기**: 관심있는 포스트 터치
5. **상호작용**: 좋아요, 댓글 작성
6. **프로필 방문**: 작성자 프로필 확인
7. **팔로우**: 관심있는 사용자 팔로우
...
### 5. 건강관리 플로우
반려동물의 건강 기록을 관리하는 과정 (건강관리 탭2에서 진입)

**단계별 설명**
1. **건강관리 접근**: 하단 네비 건강관리 탭(탭2) 선택
2. **건강 기록 확인**: 예방접종, 건강검진, 체중 기록 리스트 확인
3. **감정 트렌드 확인**: 주간/월간 감정 변화 차트 확인
4. **건강 알림 확인**: 다음 예방접종 D-day 등 알림
5. **기록 추가**: 새 건강 기록 입력 (접종, 검진, 체중 등)
6. **히스토리 조회**: 과거 기록 상세 확인
...
flowchart TD
    ...
    C -->|프로필 편집| D[프로필 편집 화면]
    C -->|반려동물 관리| D2[반려동물 목록/추가]
    C -->|내 게시글| D3[내 게시글 목록]
    C -->|채팅| D4[채팅 목록]
    C -->|저장한 글| D5[저장 목록]
    C -->|히스토리| E[감정 분석 히스토리]
