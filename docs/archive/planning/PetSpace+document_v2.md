...
### 프로젝트 성격
반려동물 케어 + 소셜 네트워킹 + AI 감정·건강 분석 서비스
...
### 핵심 기능 (MVP)
1. **AI 분석 탭 (탭3 — 중앙 FAB)**
   탭명을 "AI 분석"으로 통합하고 내부에 감정 분석 / 건강 분석 두 서브탭으로 구성.

   **공통 UX**
   - 등록된 반려동물 선택 시 이름·나이·성별·품종이 분석에 자동 반영
   - 반려동물 미등록 시 pet 정보 없이 분석 진행 (기능 제한 없음)
   - 카메라/갤러리 버튼 탭 → 딥블루 전체 화면 가이드(콘셉트1) 표시 후 촬영/선택
   - 기타 입력란 (선택, 최대 200자): 접혀있다가 탭하면 펼침
     예) "방에서 혼자 놀고 있는 상태", "산책 직후", "낯선 환경 첫 방문"
     → Gemini 프롬프트에 맥락 정보로 전달되어 분석 정확도 향상

   **감정 분석 서브탭**
   - 사진 1장 이상 업로드 (장수 제한 없음)
   - 8가지 감정 분류 (Russell + Panksepp + Ekman 이론 기반)
     happiness(기쁨), calm(편안함), excitement(흥분), curiosity(호기심),
     anxiety(불안), fear(공포), sadness(슬픔), discomfort(불편함)
   - is_sleepy: 졸음 여부 (boolean — 감정과 별도 생리 지표)
   - 스트레스 지수 / 활동량 / 편안함 지수 (0~100)
   - 부위별 신호 분석 (눈·귀·입·자세)
   - 케어 팁 2~3가지

   **건강 분석 서브탭**
   - 분석 부위 드롭다운 선택 (6가지)
     눈·귀 / 코·입 / 피부·털 / 체형(BCS) / 자세·체형 대칭 / 종합 전체
   - 부위 선택 후 카메라/갤러리 탭 시 해당 부위 전용 촬영 가이드 표시
   - 사진 1장 이상 업로드 (장수 제한 없음)
   - 종합 건강 점수 (0~100), 상태 등급 (양호/주의/위험)
   - 부위별 소견 (findings): 항목명 / 결과 / 상세 / 심각도
   - risk_alert: true 시 분석 리포트 상단에 코랄 경고 배너 + 병원찾기 연결
   - 케어 권장사항 2~3가지
   - AI 참고용 면책 문구 항상 표시

2. **감정 분석 결과 시각화**
   - 8가지 감정 차트 (바 차트, 레이더 차트)
   - 개인 AI 레포트 생성
   - 히스토리 관리 (일/주/월 단위)
...
### 부가 기능 (향후 개발)
1. **AI 분석 고도화**
   - 월간 건강 리포트 자동 생성 (감정·건강 누적 트렌드)
   - 분석 결과 기반 맞춤형 케어 루틴 제안
   - 전용 AI 모델 도입 (Gemini → 반려동물 특화 모델)
...
### 시나리오 2: AI 분석 사용 (감정·건강)
**상황**: 등록된 반려동물로 감정 또는 건강을 분석하는 경우
...
2. **반려동물 선택**
   - 등록된 반려동물 드롭다운에서 선택 → 이름·나이·성별·품종 자동 반영
   - 미등록 시 그대로 분석 진행
...
4. **사진 촬영 가이드**
   - 카메라/갤러리 버튼 탭
   - 딥블루 전체 화면 가이드 표시 (부위별 맞춤 안내)
   - 가이드 확인 후 카메라 촬영 또는 갤러리 선택

5. **기타 정보 입력 (선택)**
   - "추가 정보 입력하기" 탭해서 펼침
   - 현재 상황·환경·특이사항 최대 200자 입력
   - 예) "방금 목욕을 마쳤어요, 평소보다 조용한 상태"

6. **분석 실행**
   - 분석 시작 버튼 탭
   - Gemini 2.5 Flash 분석 (5초 이내)
   - 결과 리포트 화면으로 이동

7. **결과 확인**
   - 감정 분석: 8가지 감정 차트 + 케어 팁
   - 건강 분석: 종합 점수 + 부위별 소견 + 권장사항
   - risk_alert 시: 상단 경고 배너 + 병원찾기 바로가기
