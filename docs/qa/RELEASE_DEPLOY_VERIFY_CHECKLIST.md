...
## PHASE 7 — MY 탭 개선 (설정 일원화·펫 요약)

⛔ 배포 의존성 없음(DB 변경 없음 — 코드만). 즉시 실기기 검증 가능.

📱 **검증** (실기기 묶음)
- [ ] 설정 일원화: MY 헤더 ⚙️·내 프로필 설정 버튼 모두 **동일 설정 화면**
      (/settings/my)으로 진입(이전 2갈래 중복 제거 확인). 죽은 /profile·
      /settings(구) 진입 경로 없음.
- [ ] 설정 항목: 내 활동(반려동물/AI히스토리)·계정(계정정보 다이얼로그·프로필
      편집·알림)·정보(개인정보처리방침·커뮤니티가이드·도움말·앱정보)·로그아웃·
      회원탈퇴 노출. (리워드 항목은 숨김 상태 확인)
- [ ] **회원탈퇴 단일 동작**: 설정→회원탈퇴→'탈퇴' 입력 게이트→30일 soft
      delete 안내→실행. (중복 SettingsPage 제거로 경로 1개)
- [ ] 저장글 페이지네이션: 저장글 많을 때 스크롤하면 추가 로드(전체 일괄
      로드 아님). 저장 시각(saved_at) 내림차순.
- [ ] **내 펫 요약**: MY 상단 "내 반려동물" 가로 카드 — 아바타·이름·종/나이·
      분석 N회·최근 감정·최근 건강 기록 표시.
      · 0마리: "반려동물 등록" CTA(/pets)
      · 0분석: "분석하러 가기"(/emotion?petId= 선점 진입)
      · 0건강: 건강 행 생략
      · 헤더/뱃지/탭/그리드 기존 동작 무변경(섹션은 고정 영역 가로 스크롤)
- [ ] 디자인 토큰: MY 영역 색 일관(다크모드/테마 전환 시 surface·divider 반영).
