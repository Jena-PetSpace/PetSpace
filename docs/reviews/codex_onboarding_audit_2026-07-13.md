...
`supabase/manual_sql/history/H1_walk_records.sql`, RLS, 취급대장 트리거, 위치 약관은 준비됐고 앱의 `features/walk/`와 라우트는 아직 없다. 독립 신규 기능 후보이지만 개인위치정보를 다루므로 LBS 신고, 권한 철회, 백그라운드 수집, 보존·삭제, 실기기 검증 조건이 승인된 작업지시서가 먼저 필요하다.
