...
class KakaoMapController(
        ...
        mapView.start(
            ...
            object : KakaoMapReadyCallback() {
                override fun onMapReady(kakaoMap: KakaoMap) {
                    ...
                    // Register my_location_style with a blue circle bitmap
                    try {
                        val size = 56
                        val bmp = android.graphics.Bitmap.createBitmap(size, size, android.graphics.Bitmap.Config.ARGB_8888)
                        val cv = android.graphics.Canvas(bmp)
                        val fill = android.graphics.Paint(android.graphics.Paint.ANTI_ALIAS_FLAG)
                        fill.color = 0xFF2196F3.toInt()
                        cv.drawCircle(size / 2f, size / 2f, size / 2f * 0.75f, fill)
                        ...
                        strokePaint.strokeWidth = size * 0.1f
                        cv.drawCircle(size / 2f, size / 2f, size / 2f * 0.75f, strokePaint)
                        val myLocStyle = com.kakao.vectormap.label.LabelStyles.from(
                            "my_location_style",
                            com.kakao.vectormap.label.LabelStyle.from(bmp)
                                .setTextStyles(io.seunghwanly.kakao_maps_flutter.constant.DefaultLabelTextStyle.getTextStyle())
                        )
                        kMap.labelManager?.addLabelStyles(myLocStyle)
                    } catch (e: Exception) {
                        Log.e("KakaoMapController", "my_location_style 등록 실패: $e")
                    }
    ...
    private fun addMarker(args: JSONObject, result: MethodChannel.Result) {
        ...
        val opts = LabelOptions.from(labelOption.id, labelOption.latLng)
            .setRank(labelOption.rank)
            .setTexts(labelTextBuilder)

        if (styleId != null) {
            val registered = kMap.labelManager?.getLabelStyles(styleId)
            if (registered != null) opts.setStyles(registered)
            else opts.setStyles(io.seunghwanly.kakao_maps_flutter.constant.DefaultLabelStyle.createPinBitmap())
        } else {
            opts.setStyles(io.seunghwanly.kakao_maps_flutter.constant.DefaultLabelStyle.createPinBitmap())
        }
    ...
    private fun addMarkers(args: JSONObject, result: MethodChannel.Result) {
        ...
        for (i in 0 until options.length()) {
            ...
            if (styleId != null) {
                val registered = kMap.labelManager?.getLabelStyles(styleId)
                if (registered != null) labelOption.setStyles(registered)
                else labelOption.setStyles(io.seunghwanly.kakao_maps_flutter.constant.DefaultLabelStyle.createPinBitmap())
            } else {
                labelOption.setStyles(io.seunghwanly.kakao_maps_flutter.constant.DefaultLabelStyle.createPinBitmap())
            }
