...
class _ChatRoomSettingsPageState extends State<ChatRoomSettingsPage> {
  ...
  Future<void> _loadData() async {
    ...
    final results = await Future.wait([_loadParticipants(), _loadRoomInfo()]);
    if (mounted) {
      setState(() {
        _isLoading = false;
  ...
  Future<bool> _loadParticipants() async {
    final result = await sl<ChatRepository>().getRoomParticipants(
      widget.roomId,
    );
    return result.fold(
      (failure) {
    ...
    return Padding(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          hintText: '채팅방 이름',
          hintStyle: TextStyle(fontSize: 18.sp, color: AppTheme.hintColor),
    ...
    result.fold(
      ...
      (_) {
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('${_selectedUsers.length}명이 추가되었습니다.')),
          );
        }
      },
    );
