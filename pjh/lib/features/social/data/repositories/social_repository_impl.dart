...
class SocialRepositoryImpl implements SocialRepository {
  ...
  Future<Either<Failure, void>> likePost(String postId, String userId) async {
    try {
      ...
      return const Left(
        ServerFailure(message: '좋아요 요청을 처리하지 못했습니다. 다시 시도해 주세요.'),
      );
    }
  }
  ...
  Future<Either<Failure, Comment>> createComment(Comment comment) async {
    try {
      ...
      return const Left(
        ServerFailure(message: '댓글을 등록하지 못했습니다. 다시 시도해 주세요.'),
      );
    }
  }
  ...
  Future<Either<Failure, Follow>> followUser(
    ...
    try {
      ...
      return const Left(
        ServerFailure(message: '팔로우 요청을 처리하지 못했습니다. 다시 시도해 주세요.'),
      );
    }
  }
  ...
  Future<Either<Failure, PostLikesPage>> getPostLikesPage({
    ...
    try {
      ...
      final normalizedQuery = query.trim().replaceFirst(RegExp(r'^@+'), '');
      final page = await remoteDataSource.getPostLikesPage(
        postId: postId,
      ...
      return const Left(
        ServerFailure(message: '좋아요 목록을 불러오지 못했어요. 잠시 후 다시 시도해주세요.'),
      );
    }
  }
  ...
  Future<Either<Failure, void>> likeComment(
    ...
    try {
      ...
      return const Left(
        ServerFailure(message: '댓글 좋아요 요청을 처리하지 못했습니다. 다시 시도해 주세요.'),
      );
    }
  }
  ...
  Future<Either<Failure, void>> reportPost(
    ...
    try {
      ...
      return const Left(
        ServerFailure(message: '게시물 신고를 접수하지 못했어요. 잠시 후 다시 시도해주세요.'),
      );
    }
  }
  ...
  Future<Either<Failure, void>> reportComment(
    ...
    try {
      ...
      await remoteDataSource.reportComment(commentId, userId, reason);
      return const Right(null);
    } catch (_) {
      return const Left(
        ServerFailure(message: '댓글 신고를 접수하지 못했어요. 잠시 후 다시 시도해주세요.'),
      );
    }
  }
  ...
  Future<Either<Failure, void>> blockUser(String blockedId) async {
    try {
      ...
      return const Left(
        ServerFailure(message: '차단 요청을 처리하지 못했습니다. 다시 시도해 주세요.'),
      );
    }
  }
  ...
  Future<Either<Failure, void>> unblockUser(String blockedId) async {
    try {
      ...
      return const Left(
        ServerFailure(message: '차단 해제 요청을 처리하지 못했습니다. 다시 시도해 주세요.'),
      );
    }
  }

  @override
  Future<Either<Failure, bool>> isBlocked(String blockedId) async {
    try {
      if (!await networkInfo.isConnected) {
        return const Left(NetworkFailure(message: ErrorMessages.networkError));
      }
      return Right(await remoteDataSource.isBlocked(blockedId));
    } catch (e) {
      return const Left(ServerFailure(message: '차단 상태 확인에 실패했습니다.'));
    }
  }
  ...
  Future<Either<Failure, BlockedUsersPage>> getBlockedUsers({
    ...
    try {
      ...
      final normalized = query.trim().replaceFirst(RegExp(r'^@+'), '').trim();
      final pageLimit = limit.clamp(1, 20).toInt();
      ...
      if (ids.length != users.length) {
        return const Left(ServerFailure(message: '차단 목록 페이지 계약을 확인할 수 없습니다.'));
      }
      final nextCursor = users.length < pageLimit
          ? null
          : BlockedUsersCursor(
              blockedAt: users.last.blockedAt,
              blockId: users.last.blockId,
            );
      return Right(BlockedUsersPage(users: users, nextCursor: nextCursor));
    } catch (_) {
      return const Left(ServerFailure(message: '차단 목록을 불러오지 못했습니다.'));
    }
  }
