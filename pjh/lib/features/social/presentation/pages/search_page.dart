...
class _SearchPageState extends State<SearchPage>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (initialHashtag != null && initialHashtag.isNotEmpty) {
        _tabs.index = 1;
        _bloc.add(
          SearchPostsByHashtagRequested(hashtag: initialHashtag),
        );
  ...
  void _onPostsScroll() {
    ...
    if (state.query.startsWith('#')) {
      ...
      _bloc.add(
        SearchPostsRequested(query: state.query, loadMore: true),
      );
    }
  }
  ...
  void _dispatchSearch(String value) {
    ...
    if (query.isEmpty) {
      ...
      _tabs.animateTo(1);
      _bloc.add(
        SearchPostsByHashtagRequested(hashtag: query.substring(1)),
      );
    } else {
  ...
  Future<void> _toggleFollow(
    ...
    result.fold(
      (_) => ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('팔로우 상태를 변경하지 못했어요.')),
      ),
      (_) => _bloc.add(
        SearchFollowingChanged(
          userId: user.id,
      ...
      children: [
        ...
        if (state.users.isNotEmpty) ...[
          ...
          ...state.users.take(3).map(
                (user) => _userTile(
                  user,
                  state.followingIds.contains(user.id),
                ),
              ),
        ],
