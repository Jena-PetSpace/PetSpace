...
class _HealthAlertSettingsPageState extends State<HealthAlertSettingsPage> {
  static const bool _automaticSchedulingAvailable = false;
  ...
  Future<void> _loadSettings() async {
    ...
    if (mounted) {
      setState(() {
        _alertEnabled =
            _automaticSchedulingAvailable &&
            (prefs.getBool('health_alert_enabled') ?? false);
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('5초 뒤 테스트 알림이 도착합니다')));
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
            ...
            _buildSection(
              ...
              children: [
                _buildSwitchTile(
                  ...
                  title: '예정일 알림 받기',
                  subtitle: _automaticSchedulingAvailable
                      ? '건강 기록 예정일이 다가오면 알려드려요'
                      : '자동 예약 연결 후 제공될 예정이에요',
                  value: _alertEnabled,
                  onChanged: _automaticSchedulingAvailable
                  ...
                  _buildSection(
                    ...
                    children: [
                      _buildCheckTile(
                        ...
                        '7일 전에 알림',
                        _alertD7,
                        (v) => setState(() => _alertD7 = v),
                      ),
                      _buildCheckTile(
                        'D-3',
                        '3일 전에 알림',
                        _alertD3,
                        (v) => setState(() => _alertD3 = v),
                      ),
                      _buildCheckTile(
                        'D-1',
                        '하루 전에 알림',
                        _alertD1,
                        (v) => setState(() => _alertD1 = v),
                      ),
                    ],
                  ),
                  SizedBox(height: 16.h),
            ...
            Container(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Expanded(
                    child: Text(
                      '현재는 오른쪽 위 알림 테스트만 지원합니다.\n'
                      '건강 기록 기반 자동 예약은 운영 연결과 검증이 끝난 뒤 제공할 예정입니다.',
                      style: TextStyle(
                        fontSize: 11.sp,
  ...
  Widget _buildCheckTile(
    ...
    return CheckboxListTile(
      ...
      value: value,
      onChanged: _alertEnabled ? (v) => onChanged(v ?? value) : null,
