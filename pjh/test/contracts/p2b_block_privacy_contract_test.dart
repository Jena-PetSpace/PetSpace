...
void main() {
  ...
  test('users access is column-scoped and fresh setup matches K1', () {
    expect(
      migration,
      contains('REVOKE SELECT ON TABLE public.users FROM authenticated'),
    );
  ...
  test('social login creates the private profile only through auth uid RPC',
      () {
  ...
  test('policy helpers live outside PostgREST with RLS call permission', () {
    ...
    expect(
        migration, contains('GRANT USAGE ON SCHEMA private TO authenticated'));
    for (final helper in [
      ...
      expect(
        migration,
        contains('GRANT EXECUTE ON FUNCTION private.$helper'),
      );
    }
    expect(
      migration,
  ...
  test('notification creation and delivery both enforce the block contract',
      () {
  ...
  test('chat hidden cursor stops pagination and direct room is atomic', () {
    ...
    expect(
      migration,
      contains('GRANT UPDATE (name, description, avatar_url)'),
    );
    expect(
      migration,
      contains(
    ...
    expect(
      migration,
      isNot(contains('OR auth.uid() = user_id\n  )')),
    );
    expect(
      migration,
      contains('direct room participant limit exceeded'),
    );
    expect(
      migration,
    ...
    expect(
      migration,
      contains('participant identity and role are immutable'),
    );
    expect(migration, contains('NOT OLD.is_active AND NEW.is_active'));
    expect(migration, contains('chat_participants_update_self_state'));
    expect(
      migration,
      contains(
  ...
  test('authentication logs and failures do not expose personal data', () {
    ...
    expect(
      auth,
      contains("GeneralFailure(message: '프로필 업데이트 중 오류가 발생했습니다.')"),
    );
  });
