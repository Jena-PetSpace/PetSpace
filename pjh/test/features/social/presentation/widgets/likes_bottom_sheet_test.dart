...
void main() {
  ...
  Future<void> pumpSheet(WidgetTester tester, {int? likeCount}) async {
  ...
  testWidgets('shows the canonical count and updates clear affordance now', (
    tester,
  ) async {
    when(
      () => repository.getPostLikesPage(
    ...
    await pumpSheet(tester, likeCount: 12);
    expect(find.text('좋아요 12'), findsOneWidget);
    expect(find.byKey(const Key('likes_search_clear')), findsNothing);
    ...
    await tester.pump();
    expect(find.byKey(const Key('likes_search_clear')), findsOneWidget);

    await tester.tap(find.byKey(const Key('likes_search_clear')));
    await tester.pump();
    expect(find.byKey(const Key('likes_search_clear')), findsNothing);
  });
}
