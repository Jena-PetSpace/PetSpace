...
void main() {
  ...
  SavedPostItem item([int index = 1]) => SavedPostItem(
  ...
  testWidgets('저장 게시물을 3열 타일로 표시하고 상세로 이동한다', (tester) async {
    when(() => repository.getSavedPostsPage(
          userId: 'u1',
          scope: const SavedPostsScope.all(),
          cursor: null,
          limit: 30,
        )).thenAnswer((_) async => Right(SavedPostsPage(
          items: [item()],
          hasMore: false,
        )));
    await pump(tester);
  ...
  testWidgets('notifier move-in은 현재 로드된 window 크기로 제자리 갱신한다', (tester) async {
    ...
    when(() => repository.getSavedPostsPage(
          ...
          items: [
            item(30),
            ...List.generate(20, (index) => item(index + 31)),
          ],
          hasMore: false,
        )));
    when(() => repository.getSavedPostsPage(
    ...
    await tester.pumpAndSettle();
    verify(() => repository.getSavedPostsPage(
          userId: 'u1',
          scope: const SavedPostsScope.all(),
          cursor: null,
          limit: 50,
  ...
  testWidgets('scope 변경 전 느린 응답은 새 scope 목록을 덮어쓰지 않는다', (tester) async {
    ...
    await tester.pumpAndSettle();
    oldResponse.complete(Right(SavedPostsPage(
      items: [item(1)],
      hasMore: false,
    )));
    await tester.pumpAndSettle();
